...
<DashboardLayout title="User Management" {actions}>
	<div class="flex flex-col gap-8 pb-10">
		<!-- Page Header Info -->
		<section
			class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/5 pb-8"
		>
			<div class="space-y-1">
				...
				<p class="text-[15px] font-medium text-slate-400">
					Configure system access, organizational roles, and security permissions.
				</p>
			</div>

			<div
				class="flex items-center gap-3 px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-400 text-xs font-bold uppercase tracking-widest w-fit"
			>
		...
		<section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6 w-full">
			<!-- Role Distribution Card -->
			<div
				class="bg-slate-900/70 backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-xl hover:shadow-purple-500/10 hover:-translate-y-1 transition-all duration-300"
				aria-label="Role Distribution"
			>
				<h3
					class="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-6 border-b border-white/5 pb-4 flex items-center gap-2"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="14"
					...
					{#each roleDistribution as role}
						<div class="flex flex-col gap-2">
							...
							<div class="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
								<div
									class="h-full {role.color} rounded-full"
									style="width: {(role.count / 284) * 100}%"
								></div>
							</div>
						</div>
					{/each}
			...
			<div
				class="bg-slate-900/70 backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group"
				aria-label="Security Protocol"
			>
				<div
					class="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity pointer-events-none"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="80"
						...
						stroke-linecap="round"
						stroke-linejoin="round"
						class="text-indigo-400 rotate-12"
				...
				<h4
					class="text-[11px] font-bold text-indigo-400 uppercase tracking-widest mb-4 flex items-center gap-2"
				>
					<span class="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span>
					Security Protocol
				</h4>
				<div class="space-y-4">
					<p class="text-[13.5px] font-medium text-slate-400 leading-relaxed">
						Ensure all <span class="text-indigo-300 font-bold">Enterprise Admins</span> have 2FA enabled.
						Suspicious login attempts are automatically flagged for Master Admin review.
					</p>
					<div class="flex flex-wrap gap-3 pt-2">
						<div class="px-3 py-1.5 bg-green-500/10 border border-green-500/20 rounded-lg">
							<span class="text-[10px] font-bold text-green-500 uppercase tracking-widest"
								>SSL: Active</span
							>
						</div>
						<div class="px-3 py-1.5 bg-purple-500/10 border border-purple-500/20 rounded-lg">
					...
					<button
						class="mt-4 text-xs font-bold text-indigo-400 hover:text-white transition-all uppercase tracking-widest flex items-center gap-2 group/btn"
					>
						View Audit Logs
						<svg
							xmlns="http://www.w3.org/2000/svg"
		...
		<div
			class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/40 backdrop-blur-xl border border-white/5 p-4 rounded-2xl shadow-xl"
		>
			<SearchBar
